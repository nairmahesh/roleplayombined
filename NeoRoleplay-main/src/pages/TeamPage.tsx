...
import {
  UserPlus, MoreHorizontal, RefreshCw, ShieldOff, ShieldCheck,
  X, Check, KeyRound, ChevronDown,
} from 'lucide-react';
...
export function TeamPage() {
  ...
  const openEdit = (user: User) => {
    setEditModal({ user, role: user.role, managerId: user.managerId || '' });
    setOpenMenu(null);
  };
    ...
    try {
      ...
      setUsers(us => us.map(u =>
        u.id === editModal.user.id
          ? { ...u, role: editModal.role as User['role'], managerId: editModal.managerId || undefined }
          : u
      ));
      setEditModal(null);
  ...
  const cols = '2.5fr 1fr 1fr 1fr 0.8fr 44px';
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-white/65 mt-0.5">
            {users.length} members · manage roles and performance
          </p>
        </div>
        {(isAdmin || currentUser?.role === 'MANAGER') && (
          <button onClick={() => { setShowInvite(v => !v); setInviteResult(null); }} className="btn-primary gap-2">
      ...
      {inviteResult && (
        <motion.div
          ...
          className="card p-4 border-accent-3/30 bg-accent-3/5"
        >
          <p className="text-accent-3 font-semibold text-sm mb-2">
            {inviteResult.name} has been invited
          </p>
          <div className="flex items-center gap-3 text-sm">
            <span className="text-white/65 text-xs">Temp password:</span>
            <span className="font-mono font-bold text-accent bg-accent/10 px-2 py-0.5 rounded">
        ...
        {loading ? (
          ...
          <div className="divide-y divide-white/[0.04]">
            {users.map((u, i) => {
              ...
              const score = u.avgScore ?? (u as any).avgScore ?? 0;
              const scoreColor =
              ...
              return (
                <motion.div
                  ...
                  className="grid items-center px-5 py-3.5 hover:bg-white/[0.02] transition-colors relative"
                  style={{ gridTemplateColumns: cols }}
                >
                  <div className="flex items-center gap-3">
                    <div className={clsx(
                      'w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 bg-gradient-to-br from-accent to-accent-2',
                      u.isActive === false && 'opacity-50'
                    )}>
                      {u.firstName[0]}{u.lastName[0]}
                    </div>
                    <div>
                      <div className={clsx('text-[13.5px] font-medium', u.isActive === false && 'text-white/65')}>
                  ...
                  <div>
                    <span className={clsx('text-[10.5px] font-semibold px-2 py-1 rounded-full border', roleCfg.cls)}>
                      {roleCfg.label}
                    </span>
                  </div>

                  <div className="text-[13px] text-white/50">
                    {(u as any).sessionCount ?? (u._count as any)?.sessions ?? 0}
                  </div>

                  <div className={clsx('font-display text-[15px] font-bold', scoreColor)}>
                  ...
                  <div className="relative" onClick={e => e.stopPropagation()}>
                    {isAdmin && (
                      <button
                        onClick={() => setOpenMenu(isMenuOpen ? null : u.id)}
                    ...
                    <AnimatePresence>
                      {isMenuOpen && (
                        ...
                        >
                          <button
                            onClick={() => openEdit(u)}
                            className="w-full text-left px-4 py-2 text-[12.5px] hover:bg-white/[0.06] transition-colors flex items-center gap-2.5"
                          >
                          ...
                          >
                            {u.isActive !== false
                              ? <><ShieldOff size={12} className="text-accent-4" /> Deactivate</>
                              : <><ShieldCheck size={12} className="text-accent-3" /> Activate</>
                            }
                          </button>
                        </motion.div>
                      )}
        ...
        {editModal && (
          ...
          >
            <motion.div
              ...
              className="bg-bg-2 border border-white/[0.1] rounded-2xl w-full max-w-sm p-6 shadow-2xl"
              onClick={e => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-display text-[15px] font-bold">
                  Edit {editModal.user.firstName} {editModal.user.lastName}
                </h3>
                <button onClick={() => setEditModal(null)} className="text-white/30 hover:text-white">
                  <X size={15} />
                </button>
              </div>
              ...
              <div className="space-y-4">
                ...
                {managers.length > 0 && (
                  <div>
                    ...
                    <select
                      value={editModal.managerId}
                      onChange={e => setEditModal(m => m ? { ...m, managerId: e.target.value } : m)}
                      className="input-base"
                    >
                      <option value="">— None —</option>
                      {managers
                        .filter(m => m.id !== editModal.user.id)
                        .map(m => (
                          <option key={m.id} value={m.id}>
                            {m.firstName} {m.lastName}
                          </option>
              ...
              <div className="flex gap-2 mt-6">
                ...
                <button onClick={() => setEditModal(null)} className="btn-ghost">
                  Cancel
                </button>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-display text-[15px] font-bold flex items-center gap-2">
                  <KeyRound size={15} className="text-accent" /> Reset Password
                </h3>
                <button onClick={() => { setResetTarget(null); setResetResult(null); }} className="text-white/30 hover:text-white">
                  <X size={15} />
                </button>
              </div>
              ...
              {!resetResult ? (
                <>
                  <p className="text-sm text-white/60 mb-5">
                    ...
                    <span className="text-white font-medium">{resetTarget.firstName} {resetTarget.lastName}</span>.
                    They will need to change it after logging in.
                  </p>
                  <div className="flex gap-2">
                    <button onClick={confirmReset} disabled={resetting} className="btn-primary flex-1">
                      {resetting ? 'Resetting…' : 'Generate Password'}
                    </button>
                ...
                <>
                  <p className="text-sm text-white/60 mb-4">
                    ...
                    <span className="text-white font-medium">{resetTarget.firstName}</span>:
                  </p>
                  ...
                  <p className="text-[11px] text-white/30 mb-4">
                    Share this securely. The user should change it immediately after logging in.
                  </p>
                  <button
                    onClick={() => { setResetTarget(null); setResetResult(null); }}
