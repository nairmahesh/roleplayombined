    ...
    <div className="flex flex-col gap-4 sm:gap-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-white/80 mt-0.5">{users.length} members · manage roles and performance</p>
        </div>
        ...
        {showInvite && !inviteResult && (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div>
                <label className="text-xs text-white/80 block mb-1">First name</label>
              ...
              <div>
                <label className="text-xs text-white/80 block mb-1">Last name</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              <div className="sm:col-span-1">
                <label className="text-xs text-white/80 block mb-1">Work email</label>
              ...
              <div>
                <label className="text-xs text-white/80 block mb-1">Role</label>
              ...
              {managers.length > 0 && (
                <div>
                  <label className="text-xs text-white/80 block mb-1">Reporting Manager</label>
      ...
      {inviteResult && (
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="card p-4 border-accent-3/30 bg-accent-3/5">
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm">
            <span className="text-white/80 text-xs">Temp password:</span>
            <span className="font-mono font-bold text-accent bg-accent/10 px-2 py-0.5 rounded">{inviteResult.tempPassword}</span>
          </div>
          <p className="text-[11px] text-white/55 mt-2">Share this password securely with the new member.</p>
      ...
      <div className="card" onClick={() => setOpenMenu(null)}>
        ...
        {loading ? (
          <div className="p-8 text-center text-white/55">Loading team…</div>
        ) : (
                ...
                <motion.div key={u.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}>
                  ...
                  <div className="md:hidden px-4 py-3.5 hover:bg-white/[0.02] transition-colors" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className={clsx('text-[13px] font-medium truncate', u.isActive === false && 'text-white/80')}>{u.firstName} {u.lastName}</div>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {isAdmin && (
                          <div className="relative">
                            <button onClick={() => setOpenMenu(isMenuOpen ? null : u.id)} className="p-1.5 rounded-lg hover:bg-white/[0.08] text-white/55 hover:text-white transition-colors">
                    ...
                    <div className="flex items-center gap-2 mt-2 ml-12">
                      ...
                      <span className="text-[10px] text-white/55">{(u as any).sessionCount ?? 0} sessions</span>
                    </div>
                  </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className={clsx('text-[13.5px] font-medium', u.isActive === false && 'text-white/80')}>{u.firstName} {u.lastName}</div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setEditModal(null)}>
            <motion.div initial={{ scale: 0.96, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 10 }} className="bg-bg-2 border border-white/[0.1] rounded-2xl w-full max-w-sm p-5 sm:p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={() => setEditModal(null)} className="text-white/55 hover:text-white"><X size={15} /></button>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="text-xs text-white/80 block mb-1.5">Role</label>
                ...
                {managers.length > 0 && (
                  <div>
                    <label className="text-xs text-white/80 block mb-1.5">Reporting Manager</label>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => { setResetTarget(null); setResetResult(null); }}>
            <motion.div initial={{ scale: 0.96, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 10 }} className="bg-bg-2 border border-white/[0.1] rounded-2xl w-full max-w-sm p-5 sm:p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => { setResetTarget(null); setResetResult(null); }} className="text-white/55 hover:text-white"><X size={15} /></button>
              </div>
              {!resetResult ? (
                <>
                  <p className="text-sm text-white/75 mb-5">Generate a new temporary password for <span className="text-white font-medium">{resetTarget.firstName} {resetTarget.lastName}</span>.</p>
                  <div className="flex gap-2">
                ...
                <>
                  <p className="text-sm text-white/75 mb-4">New temporary password for <span className="text-white font-medium">{resetTarget.firstName}</span>:</p>
                  ...
                  <p className="text-[11px] text-white/55 mb-4">Share this securely. The user should change it immediately after logging in.</p>
  ...
  return (
    ...
    >
      <button onClick={() => openEdit(u)} className="w-full text-left px-4 py-2 text-[12.5px] hover:bg-white/[0.06] transition-colors flex items-center gap-2.5">
        <ChevronDown size={12} className="text-white/80" /> Edit Role &amp; Manager
      </button>
      <button onClick={() => startReset(u)} className="w-full text-left px-4 py-2 text-[12.5px] hover:bg-white/[0.06] transition-colors flex items-center gap-2.5">
        <KeyRound size={12} className="text-white/80" /> Reset Password
      </button>
